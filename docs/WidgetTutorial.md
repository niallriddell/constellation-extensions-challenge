...
2. You will have a Typescript error in your **index.tsx** file, but the
   ...
   **DefaultRowData** we can delete the first 4 lines that import **TableProps**
   ...
   You should now see a list of ratings appear in the **SummaryList** in
   Storybook.
...
2. Open up **index.tsx** and perform the following:
   ...
   Finally navigate to where the **Slider** component is used and replace the
   word Slider with StarRating:
...
1. In this task we will be adding in
   ...
   > for delaying API requests to retrieve entire lists of data until an
   > explicit request is made. It is recommended to use the view all component
   > in these situations. This component accepts the same list as the
   > SummaryList, and similarly can take actions and loading as well as props
   > related to the SearchInput component."

   First let's get the branch:

   ```bash
   git stash
   git remote set-branches --add origin 07_view_all_implementation
   git fetch origin 07_view_all_implementation
...
5. Navigate to Storybook and review the **View all** implementation. There is a
   ...
   > When adopting this pattern for Utility widgets if you have a large number
...
16. In the **Sources** tab in **Developer Tools** in the **index.tsx** tab
    ...
    As you can see a subset if these align directly with the **config.json**
...
9. Expand the Utilities panel and update or add a rating for this case.
